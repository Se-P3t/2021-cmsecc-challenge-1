...
if [-e external/generator_svp.zip]; then
  ...
  wget http://www.latticechallenge.org/svp-challenge/download/generator.zip -O generator_svp.zip
fi
...
if [-e external/generator_ideal.zip]; then
  echo "generator_ideal.zip found"
	...
	wget http://www.latticechallenge.org/ideallattice-challenge/download/generator.zip -O generator_ideal.zip
fi
...
if [-e challenge-600.bz2]; then
  echo "challenge-600.bz2 found"
	...
	wget http://www.latticechallenge.org/challenges/challenge-600.bz2 
fi
...
if [-e LWE_40_005.txt]; then
  echo "LWE_40_005.txt found"
	...
	wget https://www.latticechallenge.org/lwe_challenge/challenges/LWE_40_005.txt 
fi
